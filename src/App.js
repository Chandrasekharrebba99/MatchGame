...
import MatchGameItems from './components/MatchGameItems'
import MatchGameAllItems from './components/MatchGameAllItems'
...
class App extends Component {
  state = {
    tabsListstate: tabsList,
    imagesListstate: imagesList,
    ActivetabID: tabsList[0].tabId,
    randomImgId: '1edac278-8390-4da9-b914-5f41fb49283c',
    randomSrcImg:
      'https://assets.ccbp.in/frontend/react-js/match-game/cherry-img.png',
    score: 0,
    timer: 60,
  ...
  generateRandomNumber = () => {
    const {imagesListstate} = this.state
    const randomNumber = Math.floor(Math.random() * imagesListstate.length)
    const srcimage = imagesListstate[randomNumber]
    return srcimage
  }

  OnClickImg = id => {
    const {randomImgId, randomSrcImg, score, userScore} = this.state

    if (id === randomImgId) {
      const randomImgSrcnew = this.generateRandomNumber()

      this.setState({
        score: score + 1,
        randomSrcImg: randomImgSrcnew.imageUrl,
        randomImgId: randomImgSrcnew.id,
  ...
  getFilterditems = () => {
    const {ActivetabID, imagesListstate} = this.state
    const filterImages = imagesListstate.filter(
      arr => arr.category === ActivetabID,
    )
    return filterImages
  }
  ...
  playAgain = () => {
    const {userScore, score} = this.state

    this.setState({GameOver: false, userScore: 0, score: 0, timer: 60})
  }
  ...
  renderItems = () => {
    const {tabsListstate, randomSrcImg} = this.state
    const filterImages = this.getFilterditems()
    const result = (
      <div className="imagecontTop">
        <div>
          <img src={randomSrcImg} className="randomImg" alt="match" />
        </div>
        <ul className="rowcont">
          {tabsListstate.map(array => (
            <MatchGameItems
              key={array.tabId}
              array={array}
        ...
        <div className="bottomCont">
          <ul className="thumbnailImgcont">
            {filterImages.map(array => (
              <MatchGameAllItems
                key={array.id}
                array={array}
  ...
  render() {
    const {score, timer, GameOver, userScore} = this.state
    ...
    return (
      <div className="topbody">
        ...
        <div className="resultCont">
          {GameOver ? this.returnResult() : this.renderItems()}
        </div>
      </div>
    )
  }
